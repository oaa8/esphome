...
CONF_TRANSPARENCY = "use_transparency"
...
IMAGE_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_TRANSPARENCY): cv.boolean,
...
async def to_code(config):
    ...
    if config.get(CONF_TRANSPARENCY, None) is False and is_transparent_type:
        # TODO: Would be nice to also print the line where the error happened
        raise core.EsphomeError(
            f"Image type {config[CONF_TYPE]} must always be transparent."
        )
    transparent = config.get(CONF_TRANSPARENCY, is_transparent_type)
